...
use crate::theme::{
    COLOR_ACCENT, COLOR_MUTED, COLOR_PRIMARY, COLOR_PRIMARY_BRIGHT, COLOR_SUCCESS_BRIGHT,
...
pub fn render_settings(f: &mut Frame, area: Rect, state: &SettingsTabState) {
    ...
    let items: Vec<ListItem> = cats
        ...
        .map(|c| {
            ...
            let marker = if *c == state.selected_category {
                "▸ "
            } else {
                "  "
            };
            ...
            let value_display = if is_editing {
                ...
                match &field.kind {
                    SettingsFieldKind::Boolean => {
                        ...
                        let color = if on {
                            COLOR_SUCCESS_BRIGHT
                        } else {
                            COLOR_MUTED
                        };
                        Span::styled(icon.to_string(), Style::default().fg(color))
                    }
                    SettingsFieldKind::Dropdown { options: _ } => Span::styled(
                        format!("▾ {}", field.value),
